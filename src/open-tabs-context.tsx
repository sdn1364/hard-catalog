  ...
  try {
    ...
    return data.tabs.filter(
      (x): x is ProjectTab =>
        x != null &&
        typeof x === "object" &&
        typeof (x as ProjectTab).filePath === "string" &&
        typeof (x as ProjectTab).name === "string",
    );
  } catch {
    return [];
  }
...
export function OpenTabsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (lastWindowTitleRef.current === title) return;
    lastWindowTitleRef.current = title;
  ...
  const closeProjectTab = useCallback(
    async (filePath: string) => {
      ...
      const activeKey = activeState.filePath
        ? normalizePathKey(activeState.filePath)
        : null;
      ...
      setTabs((prev) => {
        ...
        const remaining = prev.filter(
          (t) => normalizePathKey(t.filePath) !== key,
        );
        const prevLen = prev.length;
        try {
          localStorage.setItem(
            TABS_STORAGE_KEY,
            JSON.stringify({ tabs: remaining }),
          );
        } catch {
          /* ignore */
        }
